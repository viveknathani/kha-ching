...
async function ensureExitOrder ({ initialJobData, rawFyersOrderResponse }) {
  try {
    ...
    const {
      ...
      qty,
      tradedPrice
    } = buyOrderStatus.data.orderDetails
...
async function punchOrders (initialJobData, tradingSymbol, instrumentToken) {
  ...
  try {
    ...
    console.log('🟢 [optionBuyingStrategy] trailing SL now..', {
      id,
      name,
      data
    })
...
async function checkIfStrikeTradeable ({
  instrument_token,
  from_date,
  to_date
}) {
  ...
  try {
    ...
    const { data } = await axios.post(
      `${SIGNALX_URL}/api/strat/trigger_obs`,
      props,
      {
        headers: {
...
export default async function optionBuyingStrategy (initialJobData) {
  ...
  if (
    ...
    !allowedTimes.find(
      ({ afterTime, beforeTime }) =>
        dayjs().isAfter(afterTime()) && dayjs().isBefore(beforeTime())
    )
  ) {
  ...
  const instrumentLTP = await getInstrumentPrice(
    kite,
    underlyingSymbol,
    exchange,
  ...
  const pivotStrike =
    Math.round(instrumentLTP / strikeStepSize) * strikeStepSize
  ...
  const today = (isMockOrder()
    ? getLastOpenDateSince(dayjs())
    : dayjs()
  ).format(DATE_FORMAT)
  const nearestClosedCandleTime = getNearestCandleTime(1 * 60 * 1000).format(
    DATE_TIME_FORMAT
  )
  ...
  if (!(ceTradeable || peTradeable)) {
    return Promise.reject(
      new Error('neither of call or put strikes tradeable. Will retry!')
    )
  }
